...
from risk.models import Employee, Impact, Project, ControlDomain, Standard, ControlProcess, ControlPractice, \
    ControlActivity, Selection, Scenario, ScenarioCategory, ScenarioCategoryAnswer
from .models import Company, RiskMap, RiskMapValue

# riskmap voorbeeld toevoegen bij het aanmake n/wijzigen van risk maps
...
class SelectionViewTests(TestCase):
    ...
    def setUp(self):
        self.company = Company.objects.create(name='iRiskIT')
        self.user = User.objects.create(first_name='Anita', last_name='Heiblom', username='anita', password=password)
