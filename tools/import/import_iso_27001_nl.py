...
soup = BeautifulSoup(open("iso_27001_nl.html"), "html.parser")
...
for section in sections:
    ...
    if section.h5:
        ...
        for box in boxes:
            ...
            parts = get_position_dict(all_strings, ['Beheersmaatregel', 'Implementatierichtlijn', 'Overige informatie', 'Bron:'])

            positions = parts.get('Beheersmaatregel')
            if positions:
                contr = all_strings[positions[0]:positions[1]]
            
            positions = parts.get('Implementatierichtlijn')
            if positions:
                implementation = all_strings[positions[0]:positions[1]]

            positions = parts.get('Overige informatie')
            if positions:
                information = all_strings[positions[0]:positions[1]]

            positions = parts.get('Bron:')
            if positions:
